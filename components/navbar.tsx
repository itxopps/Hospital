    ...
    <header className="sticky top-0 z-50 w-full transition-all duration-300">
      ...
      <nav className={`w-full transition-all duration-300 ${isScrolled ? "bg-white/95 backdrop-blur-md shadow-md py-2.5" : "bg-white border-b border-slate-100 py-3"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4 w-full">
          ...
          {/* Brand Logo - shrink-0 prevents name from clipping/truncating */}
          <Link href="/" className="flex items-center space-x-2.5 rtl:space-x-reverse group shrink-0">
            ...
            <div className="flex flex-col shrink-0">
              <span className="font-heading font-extrabold text-base sm:text-lg xl:text-xl text-slate-900 tracking-tight leading-none whitespace-nowrap">
                FARIS AL-JAZEERA
              </span>
              <span className="text-[9px] sm:text-[10px] xl:text-xs text-primary-700 font-medium tracking-wide whitespace-nowrap">
          ...
          <div className="hidden lg:flex items-center space-x-1 rtl:space-x-reverse xl:space-x-3">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`px-2.5 xl:px-3 py-2 rounded-lg text-xs xl:text-sm transition-all duration-200 font-medium whitespace-nowrap ${
